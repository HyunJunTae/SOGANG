...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dh1, dw2, db2 = Linear.backward(dscores, self.cache2)
        dx, dw1, db1 = Linear_ReLU.backward(dh1, self.cache1)
        
        grads['W2'] = dw2 + 2.0 * self.reg * self.params['W2']
        grads['b2'] = db2
        grads['W1'] = dw1 + 2.0 * self.reg * self.params['W1']
        grads['b1'] = db1
        ###################################################
        #                     END OF YOUR CODE                            #
        ###################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            w_name = f'W{i+1}'
            b_name = f'b{i+1}'
            self.params[w_name] = torch.randn(dims[i], dims[i+1], dtype=dtype, device=device) * weight_scale
            self.params[b_name] = torch.zeros(dims[i+1], dtype=dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            w = self.params[f'W{i}']
            b = self.params[f'b{i}']
            out, fc_relu_cache = Linear_ReLU.forward(out, w, b)
            self.caches[f'fc_relu{i}'] = fc_relu_cache
            if self.use_dropout:
                out, dropout_cache = Dropout.forward(out, self.dropout_param)
                self.caches[f'dropout{i}'] = dropout_cache
        
        # Last layer
        w = self.params[f'W{self.num_layers}']
        b = self.params[f'b{self.num_layers}']
        scores, fc_cache = Linear.forward(out, w, b)
        self.caches[f'fc{self.num_layers}'] = fc_cache
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        dout, dw, db = Linear.backward(dscores, self.caches[f'fc{self.num_layers}'])
        ...
        for i in range(self.num_layers - 1, 0, -1):
            if self.use_dropout:
                dout = Dropout.backward(dout, self.caches[f'dropout{i}'])
            dout, dw, db = Linear_ReLU.backward(dout, self.caches[f'fc_relu{i}'])
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            mask = (torch.rand_like(x) >= p).to(dtype=x.dtype, device=x.device) / (1.0 - p)
            out = x * mask
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################
