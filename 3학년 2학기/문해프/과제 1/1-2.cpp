...
vector<int> min_multiplications(int n) {
    ...
    // 2. BFS로 풀기
    // 위 알고리즘에서, 최적에 가까운 횟수를 도출했다고 가정하고 시작.
    ...
    while(limit-count != 0) {
        ...
        // 정렬
        sort(bfstemp_powers.begin(), bfstemp_powers.end());

        // 중복된 원소들을 뒤로 밀어내고, 실제 벡터 크기를 줄이기
        bfstemp_powers.erase(
            unique(bfstemp_powers.begin(), bfstemp_powers.end()), 
            bfstemp_powers.end()
        );
